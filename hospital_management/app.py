...
# Strong default seed for four hospitals and full specialization coverage
HOSPITALS = [
    'City Hospital',
...
with app.app_context():
    ...
    # Remove all previous patient-related records from the database
    Appointment.query.delete()
    Prescription.query.delete()
    Patient.query.delete()
    db.session.commit()

    # Remove legacy demo accounts from older versions and non-supported hospitals
    legacy_usernames = ['doctor1', 'doctor2', 'doctor3', 'doctor4', 'reception1', 'reception2']
    User.query.filter((~User.hospital.in_(HOSPITALS)) | User.username.in_(legacy_usernames)).delete(synchronize_session=False)
    db.session.commit()

    existing_usernames = {user.username for user in User.query.all()}

    default_users = [
        User(username='Anna Carter', password='pass123', role='receptionist', hospital='City Hospital'),
        User(username='Maya Patel', password='pass123', role='receptionist', hospital='Memorial Medical Center'),
        User(username='Lina Alvarez', password='pass123', role='receptionist', hospital='St. Mary\'s Hospital'),
        User(username='Josephine Grant', password='pass123', role='receptionist', hospital='General Hospital'),
    ]

    city_doctors = [
        'Sarah Malik', 'David Chen', 'Olivia Turner', 'Liam Hughes',
        'Avery Simmons', 'Jordan Brooks', 'Nina Flores', 'Aaron Patel',
        'Grace Lee', 'Daniel Morgan', 'Mia Carter', 'Noah Brooks'
    ]
    memorial_doctors = [
        'Priya Sharma', 'Emily Johnson', 'Marcus Reed', 'Cameron Diaz',
        'Sofia Martinez', 'Brandon Clark', 'Isla Howard', 'Kevin Wu',
        'Leah Brooks', 'Jason Park', 'Ruby Bennett', 'Jason Price'
    ]
    st_marys_doctors = [
        'Ahmed Hassan', 'Laura Gomez', 'Carlos Rivera', 'Nora King',
        'Elena Brooks', 'Mason Ward', 'Zoe Chen', 'Bria Wells',
        'Jasmine Hall', 'Ian Collins', 'Tara Owens', 'Felix Reed'
    ]
    general_doctors = [
        'Michael Brown', 'Hannah Lee', 'Robert Kim', 'Julia Nash',
        'Natalie Brooks', 'Thomas Cole', 'Meera Joshi', 'Samuel Ross',
        'Priya Menon', 'Noelle Lane', 'Victor Holt', 'Avery Monroe'
    ]

    hospital_doctors = {
        'City Hospital': city_doctors,
        'Memorial Medical Center': memorial_doctors,
        'St. Mary\'s Hospital': st_marys_doctors,
        'General Hospital': general_doctors,
    }

    for hospital, names in hospital_doctors.items():
        for name, specialization in zip(names, SPECIALIZATIONS):
            default_users.append(
                User(username=name, password='doc123', role='doctor', hospital=hospital, specialization=specialization)
            )
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Login failed. Please verify your username, password, and hospital.', 'error')
    
    return render_template('login.html', hospitals=HOSPITALS, specializations=SPECIALIZATIONS)
...
def receptionist_dashboard():
    ...
    patient_objs = Patient.query.filter_by(hospital=session['hospital']).order_by(Patient.admitted_datetime.desc()).all()
    
    # Build a display list that includes each patient’s assigned doctor
    patients = []
    for patient in patient_objs:
        appointment = Appointment.query.filter_by(patient_id=patient.id).order_by(Appointment.created_at.desc()).first()
        doctor_name = f"Dr. {appointment.doctor.username}" if appointment and appointment.doctor else 'N/A'
        patients.append({
            'name': patient.name,
            'age': patient.age,
            'doctor': doctor_name,
            'admitted_datetime': patient.admitted_datetime
        })
...
def check_doctor_availability():
    """Check if doctor is available at given time (30-minute minimum gap)"""
    data = request.get_json()
    doctor_id = data.get('doctor_id')
    appointment_time = data.get('appointment_time')
    
    try:
        requested_time = datetime.strptime(appointment_time, '%Y-%m-%dT%H:%M')
    except (ValueError, TypeError):
        return jsonify({'available': False, 'message': 'Invalid time format'})
    ...
    # Check if any existing appointment conflicts within 30-minute window
    for appt in doctor_appointments:
        try:
            existing_time = datetime.strptime(appt.appointment_time, '%Y-%m-%dT%H:%M')
            time_diff = abs((requested_time - existing_time).total_seconds() / 60)
            
            if time_diff < 30:
                return jsonify({'available': False, 'message': f'Doctor has another appointment. Need 30-minute gap.'})
        except ValueError:
            continue
    
    return jsonify({'available': True, 'message': 'Doctor is available'})
...
def admit_patient():
    ...
    # Check for double booking - ensure 30-minute gap between appointments
    try:
        requested_time = datetime.strptime(appointment_time, '%Y-%m-%dT%H:%M')
    except ValueError:
    ...
    # Check if any existing appointment overlaps within 30-minute window
    for appt in doctor_appointments:
        try:
            existing_time = datetime.strptime(appt.appointment_time, '%Y-%m-%dT%H:%M')
            time_diff = abs((requested_time - existing_time).total_seconds() / 60)
            
            # If less than 30 minutes apart, don't allow booking
            if time_diff < 30:
                existing_time_str = existing_time.strftime('%Y-%m-%d %H:%M')
                flash(f'Dr. {doctor.username} has an appointment at {existing_time_str}. Please schedule at least 30 minutes apart.', 'error')
                return redirect(url_for('receptionist_dashboard'))
        except ValueError:
            continue
